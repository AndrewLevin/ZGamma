...
f_double_muon = ROOT.TFile("/afs/cern.ch/work/a/amlevin/data/ewkzgjj/2016/double_muon.root")
f_double_eg = ROOT.TFile("/afs/cern.ch/work/a/amlevin/data/ewkzgjj/2016/double_eg.root")
f_zgjets = ROOT.TFile(" /afs/cern.ch/work/a/amlevin/data/ewkzgjj/2016/zgjets.root")
...
f_good_run_lumis=open("/afs/cern.ch/cms/CAF/CMSCOMM/COMM_DQM/certification/Collisions16/13TeV/ReReco/Final/Cert_271036-284044_13TeV_23Sep2016ReReco_Collisions16_JSON.txt")
...
#    print str(t_zgjets.run) + " " + str(t_zgjets.lumi) + " " + str(t_zgjets.event)
    ...
    if t_zgjets.gen_weight > 0:
        h_zgjets_mjj.Fill(t_zgjets.mjj,47.33*1000*35.9/zgjets_nweightedevents)
...
set_axis_fonts(h_data_mjj,"x","m_{ll} (GeV)")
set_axis_fonts(h_data_mjj,"y","Events / bin")

h_data_mjj.SetMaximum(1.05*max(h_data_mjj.GetMaximum(),h_stack.GetMaximum()))

h_data_mjj.Draw()

h_data_mjj.Print("all")

...
class exampleProducer(Module):
    ...
    def analyze(self, event):
        ...
        for i in range (0,len(electrons)):
            ...
            if (abs(electrons[i].eta + electrons[i].deltaEtaSC) < 1.479 and abs(electrons[i].dz) < 0.05 and abs(electrons[i].dxy) < 0.1) or (abs(electrons[i].eta + electrons[i].deltaEtaSC) > 1.479 and abs(electrons[i].dz) < 0.1 and abs(electrons[i].dxy) < 0.2):
